/**
 * Smart Labo Works — Whisper Provider（構造スタブ）
 *
 * IProvider（types/aiTypes.js）のインターフェースのみを実装した構造上のプレースホルダー。
 * 実際の音声→テキスト変換の実装はREFACTOR_PLAN.md Phase3（Task4の範囲外）で行う。
 ...
 */
async function chat() {
  throw new Error('Whisper Providerは未実装です（Phase3で実装予定）。');
}
...
async function testConnection() {
  return { ok: false, model: '', message: 'Whisper Providerは未実装です（Phase3で実装予定）' };
}
