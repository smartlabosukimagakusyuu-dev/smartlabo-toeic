/**
 * Smart Labo Works — Gemini Provider（構造スタブ）
 *
 * IProvider（types/aiTypes.js）のインターフェースのみを実装した構造上のプレースホルダー。
 * 実際のGemini API呼び出しはREFACTOR_PLAN.md Phase3（Task4の範囲外）で実装する。
 ...
 */
async function chat() {
  throw new Error('Gemini Providerは未実装です（Phase3で実装予定）。.env の GEMINI_API_KEY 設定だけでは利用できません。');
}
...
async function testConnection() {
  return { ok: false, model: '', message: 'Gemini Providerは未実装です（Phase3で実装予定）' };
}
